...
class Graph:
    ...
    def read_vertices_from_file(self):
        vertices = open("/home/nvidia/racecar-ws/src/racecar-controllers/deep_learning/scripts/otonomus/coor.txt", "r")
    ...
    def read_edges_from_file(self):
        edges = open("/home/nvidia/racecar-ws/src/racecar-controllers/deep_learning/scripts/otonomus/edges.txt", "r")
        if edges.mode == "r":
            contents = edges.read()
    ...
    def add_vertex(self, node):
        ...
        self.num_vertices += 1
        if len(node) > 3:
            new_vertex = Vertex(node[0], int(node[1]), int(node[2]),int(node[3]))
        else:
    ...
    def get_route(self):
        ...
        logger.log(route)
        return [x for x in route[1] if self.get_vertex(x).is_implicit == 0]
    ...
    def get_route_internal(self, p, d):
        ...
        df = self.get_shortest_path(d[1], '56')
        cost += self.get_vertex('56').distance
